...
mutable struct buyer
    ...
    duration_expectation::Vector{Float64}
    duration_expectation_history::Vector{Vector{Float64}}
    ...
    duration_of_unit_bought::Int64
    duration_of_unit_bought_history::Vector{Vector{Int64}}
       ...
       sellers = seller_price_advertising_adjustment(sellers, iter, δ, α, γ, num_buyers, variant_advertising, allow_negative_margin)
        ...
        for _buyer in buyers
            ...
            if any(available_products)
                ...
                _buyer.duration_of_unit_bought = 1 + rand(Poisson(sellers[chosen_product].duration - 1))

                push!(_buyer.duration_of_unit_bought_history, _buyer.unit_bought .*  _buyer.duration_of_unit_bought)

                product_quality = rand(Uniform(sellers[chosen_product].quality - ϵ[chosen_product], sellers[chosen_product].quality + ϵ[chosen_product]), _buyer.duration_of_unit_bought)

                _buyer.quality_of_unit_bought = mean(product_quality)
                ...
                push!(_buyer.unit_bought_history, _buyer.unit_bought)
                _buyer.duration_of_unit_bought = 0
                push!(_buyer.duration_of_unit_bought_history, fill(0, num_sellers))
        ...
        for _buyer in buyers
            ...
            if length(_buyer.neighbours) > 0
                ...
                average_duration_neighbours = reduce(+,getfield.(buyers[_buyer.neighbours], :duration_of_unit_bought) .* getfield.(buyers[_buyer.neighbours], :unit_bought)) ./ reduce(+, getfield.(buyers[_buyer.neighbours], :unit_bought))
                bought_neighbours = .!isnan.(average_quality_neighbours)
                average_quality_neighbours[.!bought_neighbours] .= 0
                average_duration_neighbours[.!bought_neighbours] .= 0
            else
                bought_neighbours = fill(0, num_sellers)
                average_quality_neighbours = fill(0, num_sellers)
                average_duration_neighbours = fill(0, num_sellers)
            end
            ...
            new_duration_expectation = _buyer.duration_expectation .+ λ_ind .* _buyer.unit_bought .* (_buyer.duration_of_unit_bought .- _buyer.duration_expectation) .+ λ_wom .* bought_neighbours .* (average_duration_neighbours .- _buyer.duration_expectation)
            ...
            _buyer.duration_expectation = new_duration_expectation

            push!(_buyer.duration_expectation_history, new_duration_expectation)
...
getfield.(sim_with_obs_12.buyers, :quality_expectation_history)[6]
...
plot(mean.(mean(getfield.(sim_with_obs_12.buyers, :duration_expectation_history))))
