...
for i in 1:9000
    ...
    push!(ex3_v16_lambda, [λ_ind, λ_wom])

    # 1.0

    ex3_v1_sim = TO_GO(2, 500, 250, λ_ind, λ_wom, "deterministic"; q = [1.0, 1.0], m = m_init, c = [0.6,0.6], ϵ = [0.33,0.33], a = [0.0, 0.0], r = [0.4, 0.4], d = d_init, q_init = [1., 1.], num_links = 1000)
    ...
    push!(ex3_v1_producer_surplus_singleton, sum.(calculate_profit_history.(ex3_v1_sim.sellers, ex3_v1_sim.function_args.γ, ex3_v1_sim.function_args.num_buyers)))
    ...
    ex3_v6_sim = TO_GO(2, 500, 250, λ_ind, λ_wom, "deterministic"; q = [1.5, 0.5], m = m_init, c = [0.6,0.6], ϵ = [0.33,0.33], a = [0.0, 0.0], r = [0.4, 0.4], d = d_init, q_init = [1.5, 0.5], num_links = 1000)
    ...
    push!(ex3_v6_producer_surplus_singleton, sum.(calculate_profit_history.(ex3_v6_sim.sellers, ex3_v6_sim.function_args.γ, ex3_v6_sim.function_args.num_buyers)))
...
scatter(getindex.(ex3_v16_lambda, 1), ex3_v1_total_surplus, markeralpha = 0.25, smooth = true)
#add_smoothing_spline(getindex.(ex3_v16_lambda, 1), Float64.(ex3_v1_total_surplus), "blue",0.50)
scatter!(getindex.(ex3_v16_lambda, 1), ex3_v6_total_surplus, markeralpha = 0.25, smooth = true)
#add_smoothing_spline(getindex.(ex3_v16_lambda, 1), Float64.(ex3_v6_total_surplus), "green")

scatter(getindex.(ex3_v16_lambda, 2), ex3_v1_total_surplus, markeralpha = 0.25, smooth = true)
#add_smoothing_spline(getindex.(ex3_v16_lambda, 2), Float64.(ex3_v1_total_surplus), "blue")
scatter!(getindex.(ex3_v16_lambda, 2), ex3_v6_total_surplus, markeralpha = 0.25, smooth = true)
#add_smoothing_spline(getindex.(ex3_v16_lambda, 2), Float64.(ex3_v6_total_surplus), "green")
